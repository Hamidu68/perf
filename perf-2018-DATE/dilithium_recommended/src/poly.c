...
{
  //if we want to generate the the accelerator for other workload, we should execute this next line
#if  (defined(LLVM_TRACE) || !defined(GEM5_HARNESS)) 
    ntt(r->coeffs); 

    #if ((DUMP_ACCEL_STATS ==2)) //in the case of debugging
      for(int i = 0; i < N; i++) {
        fprintf(f_trace_out,"ntt; r->coeffs(%d)= %d\n",i,r->coeffs[i]);
      }
    #endif
#else
  #if (ACCEL == TRACE_NTT) || (ACCEL == ALL_WORKLOADS)

    /*dump stats*/
    #if ((DUMP_ACCEL_STATS ==1))
    m5_mynewop(TRACE_NTT,1);
    m5_dump_stats(0,0);
    #endif
    ...
    // printf("************************hamid: ntt\n");
    mapArrayToAccelerator(16777218, "p", (void*)r->coeffs, N * sizeof(uint32_t));
    mapArrayToAccelerator(16777218, "zetas", (void*)zetas, N * sizeof(uint32_t) );
    printf("Invoking accelerator!\n");
    invokeAcceleratorAndBlock(16777218);
  ...
  #if ((DUMP_ACCEL_STATS ==1))
  m5_dump_stats(0,0);
  m5_mynewop(TRACE_NTT,0);
  #elif ((DUMP_ACCEL_STATS ==2))
  for(int i = 0; i < N; i++) {
    fprintf(f_gem5_out,"invntt; r->coeffs(%d)= %d\n",i,r->coeffs[i]); 
  }
  #endif


  #else  /*do not generate the accelerator file for NTT if we want to generate the accelerator for other workloads*/
    ntt(r->coeffs); 
  #endif
#endif
}
...
* Arguments:   - poly *a: pointer to input/output polynomial
**************************************************/
void poly_invntt_montgomery(poly *r) {
    //if we want to generate the the accelerator for other workload, we should execute this next line
#if  (defined(LLVM_TRACE) || !defined(GEM5_HARNESS)) 
    invntt(r->coeffs); 
    ...
    #if ((DUMP_ACCEL_STATS ==2)) //in the case of debugging
      for(int i = 0; i < N; i++) {
        fprintf(f_trace_out,"inv; r->coeffs(%d)= %d\n",i,r->coeffs[i]);
      }
    #endif
#else
  #if (ACCEL == TRACE_INVNTT) || (ACCEL == ALL_WORKLOADS)

    /*dump stats*/
    #if ((DUMP_ACCEL_STATS ==1))
    m5_mynewop(TRACE_INVNTT,1);
    m5_dump_stats(0,0);
    #endif
    ...
    /*invoke the accelerator*/
    // printf("************************hamid: invntt\n");
    mapArrayToAccelerator(16777217, "p", (void*)r->coeffs, N * sizeof(uint32_t));
    mapArrayToAccelerator(16777217, "zetas_inv", (void*)zetas_inv, N * sizeof(uint32_t) );
    printf("Invoking accelerator!\n");
    invokeAcceleratorAndBlock(16777217);
    printf("Accelerator finished!\n");
  ...
  #else  /*do not generate the accelerator file for INVNTT if we want to generate the accelerator for other workloads*/
    invntt(r->coeffs); 
  #endif
#endif
}
                      ...
                      unsigned char nonce)
{
  ...
  /*to make calls to keccak in gem5/gem5-Aladdin be consistent with native execution*/
  ctr=256; //added by hamid
  if(ctr < N) {
