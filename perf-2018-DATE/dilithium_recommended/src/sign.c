                ...
                const unsigned char *sk) 
{
  ...
  if(polyvecl_chknorm(&z, GAMMA1 - BETA)){
    ;
    // printf("if(polyvecl_chknorm(&z, GAMMA1 - BETA))--------------reject\n");
    // goto rej;
  }
  ...
  if(polyveck_chknorm(&wcs20, GAMMA2 - BETA)){
    ;
    // printf("polyveck_chknorm(&wcs20, GAMMA2 - BETA))--------------reject\n");
    // goto rej;
  }
  ...
  for(i = 0; i < K; ++i)
    for(j = 0; j < N; ++j)
      if(tmp.vec[i].coeffs[j] != w1.vec[i].coeffs[j]){
        ;
    // printf("if(tmp.vec[i].coeffs[j] != w1.vec[i].coeffs[j])--------------reject\n");
    // goto rej;
    }
  ...
  if(polyveck_chknorm(&ct0, GAMMA2 - BETA)){
    ;
    // printf(" if(polyveck_chknorm(&ct0, GAMMA2 - BETA))--------------reject\n");
    // goto rej;
  }
                     ...
                     const unsigned char *pk)
{
  ...
  if(smlen < CRYPTO_BYTES){
    smlen = 2760;
  // printf("smlen < CRYPTO_BYTES ------bad sig---------\n");  
    // goto badsig;
  }
  ...
  *mlen = smlen - CRYPTO_BYTES;
  ...
  if(polyvecl_chknorm(&z, GAMMA1 - BETA))
  {
    printf("polyvecl_chknorm(&z, GAMMA1 - BETA) -----badsig");
    // goto badsig;
  }
  ...
  for(i = 0; i < N; ++i)
    if(c.coeffs[i] != cp.coeffs[i])
      {
        ;
        // printf("if(c.coeffs[i] != cp.coeffs[i])----------badsig \n");
        // goto badsig;
      }
