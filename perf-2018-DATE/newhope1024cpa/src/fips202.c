...
void KeccakF1600_StatePermute(uint64_t * state)
{
  int round;
      ...
      loop_keccak:  for( round = 0; round < NROUNDS; round += 2 )
        {
            //    prepareTheta
...
void KeccakF1600_StatePermute2(uint64_t * state)
{
  int round;
...
void keccak_call(uint64_t *state)
{
// printf("hamid--keccak: %d\n",++num_kecc);
#if  (defined(LLVM_TRACE) || !defined(GEM5_HARNESS)) 
        KeccakF1600_StatePermute(state);
#else //call the accel if it's all the accels are attached or only this accel is supposed to be attached
  #if (ACCEL == TRACE_KECCAK) || (ACCEL == ALL_WORKLOADS)

        /*dump stats*/
        #if ((DUMP_ACCEL_STATS ==1)) //to get the cycles (stats)
        m5_mynewop(TRACE_KECCAK,1);
        m5_dump_stats(0,0);
        #elif ((DUMP_ACCEL_STATS ==2)) //use for debugging
        printf("------------------------------------------state(i) before keccak: %d ------------------------------------------\n",hamid);
        for (int i =0;i<25;i++)
        {
          printf("state(%d)= ",(hamid-1)*25+i);
          printf("%" PRIu64 ";\n", state[i]);
        }
        #endif
      
      /*invoke the accelerator*/
       mapArrayToAccelerator(16777216, "state", &state[0],25*sizeof(uint64_t));
       mapArrayToAccelerator(16777216, "KeccakF_RoundConstants", &KeccakF_RoundConstants[0],24*sizeof(uint64_t));
       printf("Invoking accelerator!\n");
       invokeAcceleratorAndBlock(16777216);
       printf("Accelerator finished!\n");

      /*dump stats*/
      #if ((DUMP_ACCEL_STATS ==1))
        m5_dump_stats(0,0);
        m5_mynewop(TRACE_KECCAK,0);
      #elif ((DUMP_ACCEL_STATS ==2))
        printf("------------------------------------------state(i) after keccak: %d ------------------------------------------\n",hamid++);
        for (int i =0;i<25;i++)
        {
          printf("state(%d)= ",(hamid-2)*25+i);
          printf("%" PRIu64 ";\n", state[i]);
        }
      #endif
...
static void keccak_squeezeblocks2(unsigned char *h, unsigned long long int nblocks,
                                 uint64_t *s, 
                                 unsigned int r
                                 )
{
...
*              - uint64_t *s:                pointer to in/output Keccak state
*              - int skip_keccak             if it's zero, call Keccak2 to number of calls to keccak in trace & gem5 be consistent; it happens when we run the program in gem5.
**************************************************/
...
void shake128_squeezeblocks2(unsigned char *output, unsigned long long nblocks, uint64_t *s)
{
  keccak_squeezeblocks2(output, nblocks, s, SHAKE128_RATE);
}
/*************************************************
* Name:        shake256
* 
