...
//    printf("---------------------------------------i:%3d\n",i);
    ...
    loop: for (k=0; k<NEWHOPE_N/2; k++){
      start = k & (distance-1);
      j = (((k & (~(distance - 1))) <<1 ) & (NEWHOPE_N-1)) + start;
      jTwiddle = j >> (i+1);

      W = omega[jTwiddle++];
      temp = a[j];
      
      //TODO: test to see if lazy reduction is effective!!!
      m1 = ~(i & 0x0001) + 1;
      m2 = (i & 0x0001) - 1;
      temp_2 = (temp + a[j + distance]);
      a[j] = ((m2)&temp_2) | ((m1)&(temp_2 % NEWHOPE_Q));
      ...
      mont = (W * ((uint32_t)temp + 3*NEWHOPE_Q - a[j + distance]));
      montgomery_reduce_inline(mont);
      a[j + distance] = mont;
    }
  }
}
