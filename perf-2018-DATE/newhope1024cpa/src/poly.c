...
void poly_uniform(poly *a, const unsigned char *seed)
{
  ...
  for(i=0;i<NEWHOPE_N/64;i++) /* generate a in blocks of 64 coefficients */
  {
    ...
    while(ctr < 64) /* Very unlikely to run more than once */ //Hamid: actullay it runs more than once in gem5
    {
...
/**************************************
 * call_ntt is used to invoke the accelerator 
 * ***********************************/
#if ((DUMP_ACCEL_STATS ==2)) //in the case of debugging
#include<stdio.h>
extern FILE *f_trace_out;
extern FILE *f_gem5_out;
#endif


void call_ntt(uint16_t * a, const uint16_t* omega)
{
    ...
    // printf("************************hamid: ntt\n");
    mapArrayToAccelerator(16777218, "a", (void*)a, NEWHOPE_N * sizeof(uint16_t));
    mapArrayToAccelerator(16777218, "omega", (void*)omega, NEWHOPE_N/2 * sizeof(uint16_t) );
  ...
  for(int i = 0; i < NEWHOPE_N; i++) {
    fprintf(f_gem5_out,"invntt; a(%d)= %d\n",i,a[i]); 
  }
  #endif
...
void poly_ntt(poly *r)
{
  ...
  call_ntt((uint16_t *)r->coeffs, omegas_bitrev_montgomery);
...
void poly_invntt(poly *r)
{
  ...
  call_ntt((uint16_t *)r->coeffs, omegas_inv_bitrev_montgomery);
